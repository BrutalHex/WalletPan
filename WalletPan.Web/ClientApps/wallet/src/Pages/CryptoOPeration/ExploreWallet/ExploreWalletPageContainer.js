...
function getTransactions(index,type, amount, walletOwner, transactionDate, destTag, fee, transactionHash) {
...
const mapStateToProps = (state, ownProps) => {
    ...
    var totalIncome= trList.filter(tr => tr.type == 'INCOME').reduce(function (result, item) {
      return result + item.amount;
    }, 0);

    var totalEXPENSE= trList.filter(tr => tr.type == 'EXPENSE').reduce(function (result, item) {
      return result + item.amount;
    }, 0);
  ...
  return {
    walletInformation:{ ...state.exploreWallet.walletInformation ,income:totalIncome, expense : totalEXPENSE},
...
const ExploreWalletPageContainer =  (connect(
  mapStateToProps,
  mapDispatchToProps
)(ExploreWalletPage))
